...
import com.evo.truco.model.{Card, Game, GameOption, Player, PlayerId}
import GameProtocol.GameResponse.{FullGameStatus, PlayerStatus}
import GameProtocol.{GameRequest, GameResponse, GameStatus}
...
object GameProtocolFormat extends ProtocolFormat with LazyLogging {
  ...
  implicit val cardEncoder: Encoder[Card] = card => Json.fromString(s"${card.rank.value}${card.suit.shortName}")
